...
async function main() {
  ...
  for (const spec of leadSpecs) {
    const lead = await prisma.lead.create({
      data: {
        ...
        email: `seed+${spec.suffix}@tradiesmate.test`,
  ...
  const demo = await prisma.client.create({
    data: {
      ...
      onboardingDivertConfirmedAt: daysAgo(9),
      onboardingStep: 6,
      ...
      allowedOrigins: ["https://tradiesmate.co.uk", "http://localhost:5173"],
  ...
  const enqAlice = await prisma.enquiry.create({
    data: {
      ...
      pipeline: "JOB",
      triage: "LIKELY_JOB",
      summary: "Boiler bang on startup — upstairs radiators freezing",
  ...
  const enqBob = await prisma.enquiry.create({
    data: {
      ...
      summary: "Bathroom suite install — wants a quote",
  ...
  const enqCara = await prisma.enquiry.create({
    data: {
      ...
      pipeline: "JOB",
      triage: "LIKELY_JOB",
      summary: "Kitchen tap dripping — wants tomorrow",
  ...
  await prisma.enquiry.create({
    data: {
      ...
      pipeline: "JOB",
      triage: "QUOTE_SHOPPER",
      summary: "Rewire quote for 3-bed terrace",
  ...
  await prisma.message.createMany({
    data: [
      ...
      },
      {
        ...
        direction: "OUTBOUND",
        channel: "EMAIL",
        toAddr: "bob.seed@tradiesmate.test",
        fromAddr: "jobs@tradiesmate.test",
  ...
  console.log("Seeding jobs (pipeline, costs, visits and history)…");
